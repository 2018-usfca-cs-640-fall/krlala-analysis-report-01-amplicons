...
**Figure 4** : Th abundance of sequences and hence bacteria seem to come from the fingertip despite there being only three samples from the fingertip.

``` r
F1_phyloseq_object <- subset_samples(phyloseq_obj,
                                     host_subject_id == "M3")
plot_bar(F1_phyloseq_object,
         x = "Order")
```

![](Analysis_Report_01_amplicons_files/figure-markdown_github/example-phyloseq-plot-5-1.png)

**Figure 5** : This data is supposed to show the distribution of abundance of bacteria according to Order for one individual, but shows the same for the entire dataset.
...
**Figure 6** : Microbiome network representation accross samples, and surface type.
...
plot\_richness(GP, measures=c("Chao1", "Shannon"))
